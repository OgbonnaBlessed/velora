...
const HotelFilters = ({ handleSubmit }) => {
    const [showFilters, setShowFilters] = useState(false);
    const [priceRange, setPriceRange] = useState(500);
    const [ratings, setRatings] = useState("");
    const [amenities, setAmenities] = useState({ freeWifi: false, pool: false, breakfast: false, parking: false });
    ...
    const handleSort = () => {
        ...
        handleSubmit({ priceRange, ratings, amenities, sortBy });
    };
            ...
            <label htmlFor={id} className="flex items-center cursor-pointer">
                <div
                    className={`relative w-4 h-4 flex items-center justify-center rounded border-2 transition-all duration-300
                        ${isChecked ? "border-[#48aadf] bg-[#48aadf]" : "border-gray-300"}`}
                >
                    <svg
                        ...
                        >
                            ...
                            <div className="flex flex-col space-y-8">
                                <div className="flex flex-col gap-8 w-full items-start">
                                    ...
                                    {/* Star Ratings Filter Section */}
                                    <div className="flex flex-col gap-2 font-Poppins font-[400]">
                                        <label className="font-Grotesk font-semibold">Star Rating</label>
                                    ...
                                    <div className="flex flex-col gap-2 font-Poppins font-[400]">
                                        <label className="font-Grotesk font-semibold">Amenities</label>
                                        <div className="flex flex-col gap-2 text-sm">
                                            {[
                                                { label: "Free WiFi", key: "freeWifi" },
                                                { label: "Swimming Pool", key: "pool" },
                                                { label: "Breakfast Included", key: "breakfast" },
                                                { label: "Free Parking", key: "parking" },
                                            ].map((option) =>
                                                renderCheckbox(
                                                    option.key,
                                                    option.label,
