...
const HotelToAirport = () => {
    ...
    const handleSearch = () => {
        ...
        if (!formData.origin) {
          newErrors.origin = 'Please select an hotel';  // Set error if origin is not selected
          hasError = true;
        }
    
        // Validate destination input
        if (!formData.destination) {
          newErrors.destination = 'Please select an airport';  // Set error if destination is not selected
          hasError = true;
        }
    
        // Check if origin and destination are the same
        if (formData.destination && formData.origin && formData.origin === formData.destination) {
          newErrors.destination = 'Hotel and airport cannot be the same';  // Set error if same
          hasError = true;
        }
        ...
        navigate('/hotel-to-airport-car-search', { state: { 
